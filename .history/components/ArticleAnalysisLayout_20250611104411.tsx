...
export default function ArticleAnalysisLayout(props: ArticleAnalysisLayoutProps) {
  ...
  if (isNewLayout(props)) {
    ...
    return (
      <div className="flex h-screen overflow-hidden md:flex-row flex-col bg-muted/30">
        {/* 左栏：原文 - 优化为60%宽度 */}
        ...
        <button
          ...
          className="absolute bottom-6 right-6 z-50 rounded-full p-3 shadow-lg bg-primary text-primary-foreground hover:shadow-xl transition-all duration-200 hover:scale-105"
        >
    ...
    return (
      <div className="relative flex h-screen overflow-hidden bg-muted/30 md:flex-row flex-col">
        {/* 左栏：原文内容 - 桌面端60%，移动端全宽 */}
        ...
        {onSendMessage && (
          <Button
            onClick={() => setDrawerOpen(true)}
            className="absolute bottom-6 right-6 z-50 rounded-full w-14 h-14 shadow-lg hover:shadow-xl transition-shadow"
            size="icon"
          >
            <MessageCircle className="h-6 w-6" />
          </Button>
